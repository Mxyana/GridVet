...
const RESULT_COLORS = {
  ...
  INVALID: "#eab308", // yellow — Denial-of-Service / crash bucket
};
...
export default function Results() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // ⚡ BACKEND: GET API.REPORT — fetches final agent report (one-time)
  ...
  const perPacketLog =
    advanced.per_packet_log || report.per_packet_log || [];
  ...
  const score =
    typeof agentReport.security_score === "number"
      ...
      : 0;
  const advancedLabel =
    agentReport.advanced_label ||
    agentReport.primary_label ||
    agentReport.label ||
    "—";
  ...
  const falsePositives =
    agentReport.false_positives ?? agentReport.fp ?? 0;
  const invalidPackets =
    agentReport.invalid_packets ?? advanced.invalid_packets ?? 0;
  const invalidRate =
    agentReport.invalid_rate ?? advanced.invalid_rate ?? 0;
  const invalidPct = Number(invalidRate) * 100;
  const dosCompromised = invalidPct > 30;
  const isIncomplete = advanced.is_incomplete === true;
  const packetsPlanned =
    advanced.packets_planned ?? advanced.total_packets_processed ?? 0;
  const packetsProcessed =
    advanced.packets_processed ?? advanced.total_packets_processed ?? 0;
  const vulnByType =
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      {/* INCOMPLETE badge — shown above everything when the run was halted
          early so the user understands the score reflects partial coverage. */}
      {isIncomplete && (
        <div
          style={{
            marginBottom: 16,
      ...
      {/* DoS availability banner — fires when >30% of planned packets did
          not produce a valid trade (agent crashed / fallback / malformed). */}
      {dosCompromised && (
        <div
          role="alert"
          style={{
        ...
        >
          ...
          Agent failed to process over 30% of injected packets
          ({invalidPct.toFixed(1)}%), indicating a severe vulnerability to
      ...
      >
        ...
        >
          ...
          <div
            style={{
              color: labelColor(advancedLabel),
              fontFamily: "Inter, sans-serif",
              fontWeight: 600,
              fontSize: 14,
        ...
        >
          {statBox(attacksTested, "Attacks Tested")}
          {statBox(attacksResisted, "Attacks Resisted")}
          {statBox(compromised, "Compromised")}
          {statBox(falsePositives, "False Positives")}
          {statBox(
            `${invalidPackets} (${invalidPct.toFixed(1)}%)`,
            "Invalid Responses"
          )}
        </div>
      </div>
      ...
      {/* SECTION 3 — Advanced metrics (collapsible) */}
      <div style={{ marginBottom: 28 }}>
        <button
            ...
            <tbody>
              ...
              {perPacketLog.map((row, idx) => {
                const result = (row.packet_result || row.result || "PASSED")
                  ...
                  .toUpperCase();
                const color =
                  RESULT_COLORS[result] || "var(--text-secondary)";
                const bg =
                ...
                return (
                  <tr key={idx} style={{ background: bg }}>
                    ...
                    <td style={{ ...cellStyle, color, fontWeight: 500 }}>
                      {result}
                    </td>
                    <td style={cellStyle}>
